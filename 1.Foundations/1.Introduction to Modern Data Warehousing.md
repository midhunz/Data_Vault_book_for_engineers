...
The story of data warehousing is a story of evolution — from tightly controlled on-prem systems to today’s scalable, cloud-native lakehouses. Each generation of architecture was built to solve a set of problems, and each introduced new challenges along the way. Understanding this evolution helps us see why **Data Vault 2.0** has become a critical part of modern data architecture.

---

### The Kimball Era — Dimensional Simplicity
...
The Kimball model used **Star** and **Snowflake** schemas — with *Fact Tables* capturing business events (like sales or transactions) and *Dimension Tables* describing the entities (like customers or products).  
...
> 🧭 *Key insight:* Kimball optimized for **usability**, not flexibility.

---

### The Inmon Era — Enterprise Consistency

At the same time, *Bill Inmon* proposed a different approach — the **Corporate Information Factory**.  
...
While Inmon’s approach ensured strong data governance and integrity, it had long delivery cycles. Business users often had to wait months to see value.
...
> 📘 *Kimball vs Inmon* — a classic debate of agility vs control.
...
“Store everything, now — figure out structure later.”  

Thus emerged the **Data Lake**.  
Organizations began dumping structured and unstructured data — logs, images, sensor feeds — into cheap storage systems like HDFS and later, cloud object stores (S3, ADLS, GCS).

The promise was freedom: schema-on-read, scale-out storage, and cost efficiency.

But freedom without control became chaos. Without metadata, quality checks, and governance, many data lakes turned into **data swamps** — filled with inconsistent, duplicated, and unreliable data.

> ⚠️ *Lesson learned:* Flexibility without structure leads to confusion, not insights.

---

### The Lakehouse — A Hybrid Evolution

Around 2018–2020, a new generation of architecture emerged — the **Data Lakehouse**.  
...
> 💡 *Visual Placeholder:*  
> Timeline diagram: `1990s (Kimball) → 2000s (Inmon) → 2010s (Data Lakes) → 2020s (Lakehouse + Data Vault)`

---

## 1.2 Why Data Vault for Modern Analytics

The modern data landscape is complex. Enterprises today manage **hundreds of systems** — ERP, CRM, HR, IoT, web, mobile apps, and external APIs. Schemas change every week, and regulators demand traceability for every transformation.  

Traditional warehouses were not built for this level of change. That’s where **Data Vault** enters.
...
A rigid star schema or fully normalized model can’t handle this gracefully.  
We need something that is **flexible, traceable, and scalable by design** — that’s what Data Vault provides.

---

### Data Vault’s Core Advantage
...
> 📍 *Example:* In a government immigration platform, a person’s visa or residency can change multiple times. Data Vault’s satellite tables naturally capture this evolving history while maintaining a consistent business key (like person ID).

---

### The Modern Context

Data Vault fits perfectly into today’s **Lakehouse architectures** because it:
- Works with **columnar storage formats** (Parquet, Iceberg, Delta).  
- Aligns with **ELT patterns** (using dbt, Spark, Trino).  
- Enables **metadata-driven automation** — Hubs, Links, Satellites can be generated directly from schema metadata.  
- Scales linearly with cloud compute.

> 💡 *Visual Placeholder:*  
> Architecture diagram showing:  
> *Source → Raw Layer → Raw Vault → Business Vault → Star Schema / Dashboard.*
...
It’s not just a modeling technique — it’s a **complete methodology** that combines modeling, architecture, and agile principles.
...
2. **Auditability and Traceability**  
   ...
   Nothing is overwritten — full history is preserved.
...
|------------|----------|---------|
| **Hub** | Stores unique business keys — core identifiers in the business domain. | `HUB_CUSTOMER (CUSTOMER_ID, LOAD_DTS, SRC)` <br> `HUB_PRODUCT (PRODUCT_ID, LOAD_DTS, SRC)` |
| **Link** | Captures relationships between Hubs — how entities interact. | `LNK_SALE (CUSTOMER_ID, PRODUCT_ID, STORE_ID, SALE_ID, LOAD_DTS)` |
| **Satellite** | Stores descriptive attributes and their changes over time — the context. | `SAT_CUSTOMER_DETAILS (CUSTOMER_ID, NAME, EMAIL, PHONE, HASHDIFF, LOAD_DTS)` <br> `SAT_PRODUCT_PRICING (PRODUCT_ID, PRICE, DISCOUNT, EFFECTIVE_DATE, HASHDIFF, LOAD_DTS)` |
...
> `HUB_CUSTOMER` ↔ `LNK_SALE` ↔ `HUB_PRODUCT` ↔ `SAT_PRODUCT_PRICING`
...
- **Satellites** hold the descriptive and historical data — for example, price changes, customer contact details, or store addresses over time.  
- **PIT/Bridge tables** are used to simplify querying — such as daily summaries or recent customer activity snapshots.  

Together, these components form the **core building blocks** of a Data Vault — providing both flexibility (easily add new satellites) and traceability (never overwrite history).

> 💡 *Visual Placeholder:*  
> ERD with one Hub → one Link → one Satellite.
...
**In short:**  
> Data Vault 2.0 enables you to design a **scalable, auditable, and adaptive** data warehouse that works for today’s cloud-native, fast-changing environments.

---

### Mini Exercise

Draw your company’s data flow from *Source → Transformation → Reports*.  
Ask yourself:
- Where is the “truth” of your data defined?  
- Where can you trace a report back to its raw source?  

Where you can’t — that’s where Data Vault helps.
